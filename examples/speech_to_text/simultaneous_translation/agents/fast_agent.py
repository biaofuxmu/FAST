...
from examples.simultaneous_translation.models.fast_simulst import *
...
class FairseqSimulSTAgent(SpeechAgent):
    ...
    def add_args(parser):
        ...
        parser.add_argument("--w2v2-model-path", type=str, default=None,
                            help="wav2vec2.0 path")
        ...
        parser.add_argument("--threshold", type=float, default=1, 
                            help="cif threshold")
        parser.add_argument("--future-num", type=int, default=0, 
                            help="future number of the cnn tokens in wav2vec2")
        parser.add_argument("--simul-mode",type=str,
                            default="vanilla",
                            choices=["vanilla", "fai"],
                            help="if True, perform streaming inference")
        parser.add_argument("--fast-cif", default=True, action="store_true")
    ...
    def load_model_vocab(self, args):
        ...
        sys.stderr.write(
            "streaming st model path: " + filename + "\n")
        ...
        if args.waitk_lagging is not None:
            ...
            sys.stderr.write(
                "waitk_lagging: " + str(state["cfg"]["model"].__dict__["waitk_lagging"]) + "\n")
        if args.fixed_pre_decision_ratio is not None:
            ...
            sys.stderr.write(
                "fixed_pre_decision_ratio: " + str(state["cfg"]["model"].__dict__["fixed_pre_decision_ratio"]) + "\n")
        if args.w2v2_model_path is not None:
            state["cfg"]["model"].__dict__["w2v2_model_path"] = args.w2v2_model_path
            sys.stderr.write(
                "w2v2_model_path: " + str(state["cfg"]["model"].__dict__["w2v2_model_path"]) + "\n")
        if args.user_dir is not None:
            state["cfg"]["task"].__dict__["user_dir"] = args.user_dir
            sys.stderr.write(
                "user_dir: " + str(state["cfg"]["task"].__dict__["user_dir"]) + "\n")
        if args.future_num is not None:
            state["cfg"]["model"].__dict__["future_num"] = args.future_num
            sys.stderr.write(
                "future_num: " + str(state["cfg"]["model"].__dict__["future_num"]) + "\n")
        if args.simul_mode is not None:
            if args.simul_mode == "vanilla":
                assert args.future_num == 0
            state["cfg"]["model"].__dict__["simul_mode"] = args.simul_mode
            sys.stderr.write(
                "simul_mode: " + str(state["cfg"]["model"].__dict__["simul_mode"]) + "\n")
        if args.fast_cif is not None:
            state["cfg"]["model"].__dict__["fast_cif"] = args.fast_cif

        
        self.audio_len = 20 * 16 * args.future_num
        sys.stderr.write("wait audio_len: " + str(self.audio_len) + "\n")
    ...
    def update_model_encoder(self, states):
        ...
        segment_nums = states.units.source.value.size(0)

        if (self.args.simul_mode.lower() == "vanilla" or \
            self.args.simul_mode.lower() == "fai" or \
            segment_nums > self.audio_len or states.finish_read()):
            states.encoder_states = self.model.encoder(src_indices, src_lengths, finish_read=states.finish_read())
    ...
    def update_states_decode(self, states):
        if self.lang is None:
            ...
            if len(states.units.target.value) < 1:
                tgt_indices = self.to_device(
                    torch.LongTensor(
                        [self.model.decoder.dictionary.eos()]
                        + [x for x in states.units.target.value if x is not None]
                ...
                _, _ = self.model.decoder.forward(
    ...
    def policy(self, states):
        ...
        if states.encoder_states["alphas"].sum().cpu().numpy() < 1 and not states.finish_read():
            return READ_ACTION
        if states.incremental_states["steps"]["src"] < self.prev_src:  
            # print('states.units.target', states.units.target)
            if not states.finish_read():
                return READ_ACTION
            elif len(states.units.target) < 2: 
                self.update_states_decode(states)
                return WRITE_ACTION
            elif states.units.target[-2] != states.units.target[-1]: 
                self.update_states_decode(states)
                return WRITE_ACTION
            else:
                return None
