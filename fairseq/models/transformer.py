...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def extract_features_scriptable(
        ...
        sampled_k: Optional[Tensor] = None
        if encoder_out is not None:
            ...
            sampled_k = encoder_out.get("sampled_k", None)
        ...
        if sampled_k is not None:
            def get_mask(tgt_len, src_len, sampled_k, device):
                attn_mask = []
                for k in sampled_k:
                    attn_mask.append(torch.triu(torch.zeros(tgt_len, src_len, device=device).fill_(float('-inf')), k))
                return torch.stack(attn_mask, dim=0)
            cross_attn_mask = get_mask(slen, enc.size(0), sampled_k, enc.device) #.repeat_interleave(self.args.decoder_attention_heads, dim=0)
...
@register_model_architecture("transformer", "transformer_wmt_en_de_mid")
