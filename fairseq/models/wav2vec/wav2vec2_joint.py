...
from fairseq.models.wav2vec import Wav2Vec2Model, base_architecture
MASKING_DISTRIBUTION_CHOICES = ChoiceEnum(["static", "uniform", "normal", "poisson"])
# import pdb; pdb.set_trace()

@register_model("wav2vec2_joint")
class Wav2Vec2JointModel(Wav2Vec2Model):
    @staticmethod
    def add_args(parser):
        """Add model-specific arguments to the parser."""
        super(Wav2Vec2JointModel, Wav2Vec2JointModel).add_args(parser)
    ...
    def forward(self, source, padding_mask=None, mask=True, features_only=False, finish_read=True):
        ...
        if self.simul_mode is not None: # streaming inference
            if finish_read or self.simul_mode.lower() == "vanilla":
                ...
                if self.simul_mode.lower() == "fine-mask":
                    ...
                    x = self.encoder(x, padding_mask=padding_mask)[:, :padding_mask_len.max().item(), :]
                    token_len = x.size(1)
                    padding_mask = torch.arange(token_len).unsqueeze(0).to(x.device) >= padding_mask_len.unsqueeze(-1)
                elif self.simul_mode.lower() == "fine-wait":
                    x = self.encoder(x, padding_mask=padding_mask)
                    token_len = x.size(1)
                    padding_mask = torch.arange(token_len).unsqueeze(0).to(x.device) >= torch.clamp(padding_mask_len - future_len, min=1).unsqueeze(-1)
                    max_x_len = ((~padding_mask).sum(dim=-1)).max().item()
                    x = x[:, :max_x_len, :]
                    padding_mask = padding_mask[:, :max_x_len]

        else:
            mask_output, sampled_lengths_mask, mask_mask = self.get_output_for_sampled_wait_and_mask(
                full_speech_tokens=x.clone(), padding_mask=padding_mask)

            # If we put full speech here, we can return it in partial for possible decoder kd.
    ...
    def get_output_for_sampled_wait_and_mask(self, full_speech_tokens, padding_mask):
        ...
        future_mask_length = 50  # torch.randint(5, 50)
        ...
        if padding_mask is not None:
            ...
            if not self.training and self.dev_sample_half_speech:
                sampled_lengths = torch.div(speech_tokens_lengths, 2, rounding_mode="trunc")
            else:
        ...
        mask_output = self.encoder(mask_speech_tokens, padding_mask=~final_lengths_mask)

        return mask_output, sampled_lengths_mask, mask_mask
