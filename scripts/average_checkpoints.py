...
def last_n_checkpoints(paths, n, update_based, upper_bound=None):
    ...
    if update_based:
        ...
        # pt_regexp = re.compile(r"checkpoint\.best_accuracy_(\d+)\.(\d+)\.pt")
...
def best_checkpoints(paths, epoch_list):
    assert len(paths) == 1
    path = paths[0]
    epoch_list = [int(epoch) for epoch in epoch_list]
    checkpoint_list = [f"checkpoint{epoch}.pt" for epoch in sorted(epoch_list, reverse=True)]
    return [os.path.join(path, ckpt) for ckpt in checkpoint_list]
...
def main():
    ...
    num_group.add_argument('--num-update-checkpoints', type=int,
                           ...
                           'and average last this many of them.')
    num_group.add_argument('--list-epoch-checkpoints', nargs='+',
                           help='if set, will try to find checkpoints with names checkpoint_xx.pt in the path specified by input, '
                           'and average last this many of them.')
    ...
    if num is not None:
        ...
        args.inputs = best_checkpoints(args.inputs, args.list_epoch_checkpoints)
